    ...
    public void ShowTab(int tabIndex)
    {
        for (int i = 0; i < tabs.Length; i++)
        {
            if (i == tabIndex)
            {
                ...
                tabButtons[i].GetComponent<Image>().sprite = selectedButtonSprite; // Cambia la imagen de fondo del botón a seleccionado
            }
            else
            {
                tabs[i].SetActive(false); // Oculta las demás pestañas
                tabButtons[i].GetComponent<Image>().sprite = unselectedButtonSprite; // Cambia la imagen de fondo del botón a no seleccionado
            }
        }
