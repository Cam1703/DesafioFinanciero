...
public class BancoManager : MonoBehaviour
{
    ...
    public void GetInputCuentaAhorrosOnClick(Button button)
    {
        ...
        {
            // Solo montos positivos y que el jugador realmente tenga: un monto negativo
            // aumentaba los ahorros, y uno mayor al disponible los dejaba en negativo.
            if (depositoInicialCuentaAhorros <= 0 || depositoInicialCuentaAhorros > presupuestoManager.Ahorros)
            {
                Debug.Log("Deposito invalido: debe ser positivo y no mayor a los ahorros disponibles");
    ...
    public void GetInputPlazoFijoOnClick(Button button)
    {
        if (button.GetComponentInChildren<TMP_Text>().text == "Adquirir")
        {
            ...
            {
                // Solo montos positivos y con fondos suficientes (mismo motivo que la cuenta de ahorros).
                if (depositoInicialPlazoFijo <= 0 || depositoInicialPlazoFijo > presupuestoManager.Ahorros)
                {
                    Debug.Log("Deposito invalido: debe ser positivo y no mayor a los ahorros disponibles");
                ...
                // El descuento se hace solo si el monto fue valido: antes ocurria fuera
                // del if, asi que un texto no numerico descontaba el ultimo valor parseado.
